...
def assemble(instructions, labels, data_labels, __GLOBAL_ADDR__=0):
    ...
    for instr in instructions:
        ...
        # ST $rs, IMM($rt) or ST $rs, IMM (implicit zero addressing)
            ...
            pseudoinstr = f"{opcode1}|{rs}|{rs}|{imm[:18]}    (I-Type | Pseudo JAL)"
            ...
            pseudo = f"{opcode1}|{rs}|{rt}|00{imm[16:]}    (I-Type | Pseudo LA)"
            instruction = f"{opcode2}|{rs}|{rt}|00{imm[:16]}    (I-Type | Pseudo LA)"
